...
const Reports = () => {
    ...
    const getData = async () => {
        const res = await axiosInterceptorInstance.get( ENDPOINTS.saleReportData + startDate + '/' + endDate);
        if (res.status == 200) {
    ...
    const processData = (data) => {
        ...
        data.map(orden => {
            ...
            processedData.push(
                getOrdenJSON(orden.id, orden.product.name, orden.order.user.name, orden.product.updated_at.split('T')[0], orden.order.total));
        });
    ...
    const generatePdf = () => {
        let pdfInfo = { title: "Reporte de ventas", fileName: "reporte_ventas" };
        let filters = [
            {
                name: "Fecha inicio",
                value: startDate
        ...
        const tableHeads = ["Venta", "Cliente", "Fecha", "Total"];
